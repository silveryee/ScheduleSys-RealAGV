...
# 仿真脚本作为socketio 客户端与服务器通信，缺点是无法与服务器共用glob文件中的变量，需要发送地图数据
...
def send_data():
    ...
    while running_flag:
        if not send_queue.empty():
            ...
            # EDGE_INFO的values是edge对象，无法解析为json数据，所以传输EDGE_LENGTHS
            sio.emit('simScriptRequest', (agv_dynamic_data, cmds, glob.EDGE_LENGTHS, glob.NODE_ATTRIBUTES),
                     namespace='/simScript')
...
@sio.on('simScriptResult', namespace='/simScript')
def message(sid, data):
    print('Message received from', sid, ':', data)
    # 将收到的消息放入队列中
    agv_dynamic_data_queue.put(data)
